...
namespace Boomlagoon.JSON {
	...
	public class JSONValue {
		...
		public JSONValue(JSONObjectBoom obj) {
			if (obj == null) {
		...
		public JSONValue(JSONValue value) {
			...
			switch (Type) {
				...
				case JSONValueType.Object:
					if (value.Obj != null) {
						Obj = new JSONObjectBoom(value.Obj);
					}
					break;
		...
		public JSONObjectBoom Obj { get; set; }
		...
		public static implicit operator JSONValue(JSONObjectBoom obj) {
	...
	public class JSONArray : IEnumerable<JSONValue> {
		...
		public static JSONArray Parse(string jsonString) {
			var tempObject = JSONObjectBoom.Parse("{ \"array\" :" + jsonString + '}');
	...
	public class JSONObjectBoom : IEnumerable<KeyValuePair<string, JSONValue>> {
		...
		public JSONObjectBoom() {
		}

		/// <summary>
		/// Construct a copy of the given JSONObjectBoom.
		/// </summary>
		/// <param name="other"></param>
		public JSONObjectBoom(JSONObjectBoom other) {
		...
		public JSONObjectBoom GetObject(string key) {
			var value = GetValue(key);
			if (value == null) {
				Debug.LogError(key + " == null");
				return null;
		...
		/// <summary>
		/// Attempt to parse a string into a JSONObjectBoom.
		/// </summary>
		/// <param name="jsonString"></param>
		/// <returns>A new JSONObjectBoom or null if parsing fails.</returns>
		public static JSONObjectBoom Parse(string jsonString) {
			...
			for (var startPosition = 0; startPosition < jsonString.Length; ++startPosition) {
				...
				switch (state) {
					case JSONParsingState.Object:
						...
						JSONValue newObj = new JSONObjectBoom();
		...
		private static JSONObjectBoom Fail(char expected, int position) {
			return Fail(new string(expected, 1), position);
		}

		private static JSONObjectBoom Fail(string expected, int position) {
			Debug.LogError("Invalid json string, expecting " + expected + " at " + position);
			return null;
		}

		/// <returns>String representation of this JSONObjectBoom</returns>
		public override string ToString() {
			var stringBuilder = new StringBuilder();
			stringBuilder.Append('{');
		...
		/// <summary>
		/// Empty this JSONObjectBoom of all values.
		/// </summary>
		public void Clear() {
			values.Clear();
		}
