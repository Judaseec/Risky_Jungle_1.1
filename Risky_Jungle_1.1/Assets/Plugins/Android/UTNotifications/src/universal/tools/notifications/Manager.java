...
//public
	public static boolean initialize(boolean googlePlayPushNotificationsEnabled, boolean amazonPushNotificationsEnabled, String googlePlaySenderID, boolean willHandleReceivedNotifications, int startId, boolean incrementalId, int showNotificationsMode, boolean restoreScheduledOnReboot, int notificationsGroupingMode) {
...
//package
	static void postNotification(Context context, String title, String text, int id, Bundle userData, String notificationProfile, boolean repeatedOrNotScheduled) {
		postNotificationPrepared(context, prepareNotification(context, title, text, notificationProfile, userData), id, userData, repeatedOrNotScheduled);
	}
	...
	static void scheduleNotificationCommon(Context context, boolean repeated, int triggerInSeconds, int intervalSeconds, String title, String text, int id, Bundle userData, String notificationProfile) {
		...
		if (repeated) {
			...
			alarmManager.setInexactRepeating(AlarmManager.ELAPSED_REALTIME, elapsedRealtime + triggerInSeconds*1000,  intervalSeconds*1000, pendingIntent);
		} else {
			alarmManager.set(AlarmManager.ELAPSED_REALTIME, elapsedRealtime + triggerInSeconds*1000, pendingIntent);
		}
...
//private
	...
	private static Notification prepareNotification(Context context, String title, String text, String notificationProfile, Bundle userData) {
	    ...
	    if (UnityPlayer.currentActivity != null) {
	    	...
	    	try {
	    		mainActivityClass = Class.forName(context.getSharedPreferences(Manager.class.getName(), Context.MODE_PRIVATE).getString(MAIN_ACTIVITY_CLASS_NAME, "com.unity3d.player.UnityPlayerNativeActivity"));
	...
	private static String packReceivedNotification(int id, Bundle userData) {
		try {
			...
			userData.remove(KEY_TITLE);
			userData.remove(KEY_TEXT);
			userData.remove(KEY_NOTIFICATION_PROFILE);
	...
	private static final String MAIN_ACTIVITY_CLASS_NAME = "MAIN_ACTIVITY_CLASS_NAME";
