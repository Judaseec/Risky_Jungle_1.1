    ...
    //protected
        protected void LateUpdate()
        {
            ...
            if (m_willHandleReceivedNotifications && OnNotificationsReceivedHasSubscribers())
            {
                ...
                List<ReceivedNotification> list = new List<ReceivedNotification>(localCount + remoteCount);
                ...
                if (localCount > 0)
                {
                    foreach (var notification in NotificationServices.localNotifications)
                    {
                        ...
                        list.Add(new ReceivedNotification(title, text, notification.userInfo.Contains(m_idKeyName) ? int.Parse((string)notification.userInfo[m_idKeyName]) : -1, userData, ExtractNotificationProfile(notification.soundName)));
                    }
                }
                ...
                if (remoteCount > 0)
                {
                    foreach (var notification in NotificationServices.remoteNotifications)
                    {
                        ...
                        list.Add(new ReceivedNotification(title, text, m_nextPushNotificationId, userData, ExtractNotificationProfile(notification.soundName)));
